...
typedef void(GSUI::*FrameCallback)(Lcd *display, DisplayUiState* state, int16_t x, int16_t y);
typedef void(GSUI::*OverlayCallback)(Lcd *display, DisplayUiState* state);
typedef void(*LoadingDrawFunction)(Lcd *display, LoadingStage* stage, uint8_t progress);

class DisplayUi {
	public: Lcd             *display = nullptr;
  private:
	
	
	GSUI* p_gsuiInstance=nullptr;
  ...
  public:
    ...
    DisplayUi();
	void setDisplayInstance(Lcd *display);
	void setGsuiInstance(GSUI* pgsuiInstance);
    /**
