 ...
 */
uint16_t Eeprom::CalculateBytesToWrite(uint16_t size, uint16_t offset)
{
 ...
 */
void Eeprom::WriteToEeprom(uint16_t page, uint16_t offset, uint8_t* data, uint16_t size)
{
    ...
    for (int i = 0; i < numberOfPages; i++)
    {
        ...
        hal.I2cMemWrite(EEPROM_I2C_HANDLE, EEPROM_I2C_ADDRESS, memoryAddress, 2, &data[dataPosition], bytesRemaining, 1000);
 ...
 */
void Eeprom::FloatToByteArray(uint8_t* byteArray, float floatValue)
{
    union {
        float floatVal;
        uint8_t bytes[4];
 ...
 */
float Eeprom::ByteArrayToFloat(uint8_t* byteArray)
{
    union {
        float floatVal;
        uint8_t bytes[4];
 ...
 */
void Eeprom::WriteFloatToEeprom(uint16_t page, uint16_t offset, float data)
{
 ...
 */
float Eeprom::ReadFloatFromEeprom(uint16_t page, uint16_t offset)
{
    uint8_t buffer[4];
 ...
 */
void Eeprom::ReadFromEeprom(uint16_t page, uint16_t offset, uint8_t* data, uint16_t size)
{
    int pageAddressShift = log(EEPROM_PAGE_SIZE) / log(2);
    ...
    for (int i = 0; i < numberOfPages; i++)
    {
        uint16_t memoryAddress = (startPage << pageAddressShift) | offset;
        uint16_t bytesRemaining = CalculateBytesToWrite(size, offset);
        hal.I2cMemRead(EEPROM_I2C_HANDLE, EEPROM_I2C_ADDRESS, memoryAddress, 2, &data[dataPosition], bytesRemaining, 1000);
 ...
 */
void Eeprom::EraseEepromPage(uint16_t page)
{
    ...
    hal.I2cMemWrite(EEPROM_I2C_HANDLE, EEPROM_I2C_ADDRESS, memoryAddress, 2, eraseBuffer, EEPROM_PAGE_SIZE, 1000);
